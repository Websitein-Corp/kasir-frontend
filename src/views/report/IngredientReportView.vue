<template>
  <div v-if="!page.loading">
    <PageContainer
      title="Laporan Riwayat Bahan Baku"
      subtitle="Daftar Pergerakan Stok Bahan Baku"
    >
...
const processData = async () => {
  if (table.filters.date.start && table.filters.date.end) {
    const { data } = await axios.get(
      `${process.env.VUE_APP_API_BASE_URL}/api/ingredients/history?shop_id=${auth.shopId}&date_start=${table.filters.date.start}&date_end=${table.filters.date.end}&keyword=${table.filters.keyword}&page=${table.page.current}&download=0`,
      {
        headers: {
...
const handleDownload = async () => {
  await axios
    .get(
      `${process.env.VUE_APP_API_BASE_URL}/api/ingredients/history?shop_id=${auth.shopId}&date_start=${table.filters.date.start}&date_end=${table.filters.date.end}&keyword=${table.filters.keyword}&page=${table.page.current}&download=1`,
      {
        headers: {
          Authorization: `Bearer ${auth.authToken}`,
        },
